...
func TSharkPath() (string, *gowid.KeyValueError) {
	...
	if tsharkBin != "" {
		...
		if !IsCommandInPath(tsharkBin) {
			...
			errstr := fmt.Sprintf("Could not find tshark in your PATH. The tshark binary is required to run pcaptui.\n")
...
// DefaultPcapDir returns ~/.cache/pcaps by default. Pcaptui will check a
...
// pcaptui - this invokes pcaptui in a special mode where it
// first tries DumpcapBin, then if that fails, TSharkBin - for
// the best of both worlds. To detect this, pcaptui will run
// CaptureBin with PCAPTUI_CAPTURE_MODE=1 in the environment,
// so when pcaptui itself is invoked with this in the environment,
...
// pcaptui's default capture method is to run dumpcap and tshark
// as a fallback. I don't want to tell the user the capture binary
// is pcaptui - that'd be confusing. We know that on Linux, pcaptui
...
func PrunePcapCache() error {
	...
	log.Infof("Pruning pcaptui's pcap disk cache at %s...", PcapDir())
