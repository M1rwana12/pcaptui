...
import (
	...
	"github.com/m1rwana12/pcaptui/widgets/copymodetable"
	"github.com/m1rwana12/pcaptui/widgets/enableselected"
	"github.com/m1rwana12/pcaptui/widgets/keepselected"
	"github.com/m1rwana12/pcaptui/widgets/scrollabletable"
...
func openConvsUi(app gowid.IApp) {
	...
	newSize, reset := pcaptui.FileSizeDifferentTo(Loader.PcapPdml, convsPcapSize)
	if reset {
		convsView = nil
	}
...
func (w *ConvsUiWidget) construct() {
	...
	w.shortNames = pcaptui.ConvTypes()
...
func (w *ConvsUiWidget) OnData(data string, app gowid.IApp) {
	...
	for scanner.Scan() {
		...
		if err == nil && n == 1 {
			...
			ports = pcaptui.StringInSlice(cur, []string{"UDP", "TCP"})
			ipv6 := (cur == "IPv6")

			var addrComp table.ICompare = pcaptui.IPCompare{}
			if pcaptui.StringInSlice(cur, []string{"Ethernet"}) {
				addrComp = pcaptui.MACCompare{}
			}

			var convComp table.ICompare = pcaptui.ConvPktsCompare{}
