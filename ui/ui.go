...
import (
	...
	"github.com/m1rwana12/pcaptui/pkg/noroot"
	...
	"github.com/m1rwana12/pcaptui/ui/tableutil"
	"github.com/m1rwana12/pcaptui/widgets"
	"github.com/m1rwana12/pcaptui/widgets/appkeys"
	"github.com/m1rwana12/pcaptui/widgets/copymodetree"
	"github.com/m1rwana12/pcaptui/widgets/enableselected"
	"github.com/m1rwana12/pcaptui/widgets/expander"
	"github.com/m1rwana12/pcaptui/widgets/filter"
	"github.com/m1rwana12/pcaptui/widgets/hexdumper2"
	"github.com/m1rwana12/pcaptui/widgets/ifwidget"
	"github.com/m1rwana12/pcaptui/widgets/mapkeys"
	"github.com/m1rwana12/pcaptui/widgets/minibuffer"
	"github.com/m1rwana12/pcaptui/widgets/resizable"
	"github.com/m1rwana12/pcaptui/widgets/rossshark"
	"github.com/m1rwana12/pcaptui/widgets/search"
	"github.com/m1rwana12/pcaptui/widgets/withscrollbar"
...
var keyState pcaptui.KeyState
var marksMap map[rune]pcaptui.JumpPos
var globalMarksMap map[rune]pcaptui.GlobalJumpPos
var lastJumpPos int

var NoGlobalJump pcaptui.GlobalJumpPos // leave as default, like a placeholder
...
func init() {
	...
	marksMap = make(map[rune]pcaptui.JumpPos)
	globalMarksMap = make(map[rune]pcaptui.GlobalJumpPos)
	lastJumpPos = -1
...
func (g getMappings) Get() []pcaptui.KeyMapping {
	return pcaptui.LoadKeyMappings()
}

func (g getMappings) None() bool {
	return len(pcaptui.LoadKeyMappings()) == 0
}
...
func UpdateProgressBarForFile(c *pcap.PacketLoader, prevRatio float64, app gowid.IApp) float64 {
	...
	if psml, ok := c.PcapPsml.(string); ok && c.PsmlLoader.IsLoading() {
		c.PsmlLoader.Lock()
		c2, m, err = system.ProcessProgress(pcaptui.SafePid(c.PsmlCmd), psml)
		c.PsmlLoader.Unlock()
		if err == nil {
...
func makeStructNodeDecoration(pos tree.IPos, tr tree.IModel, wmaker tree.IWidgetMaker) gowid.IWidget {
	...
	if level < 0 {
		panic(errors.WithStack(gowid.WithKVs(pcaptui.BadState, map[string]interface{}{"level": level})))
	}
	...
	if !ok {
		panic(errors.WithStack(gowid.WithKVs(pcaptui.BadState, map[string]interface{}{"tree": tr})))
	}
...
var (
	_ pcaptui.ICommandOutput     = urlCopiedCallbacks{}
	_ pcaptui.ICommandError      = urlCopiedCallbacks{}
	_ pcaptui.ICommandDone       = urlCopiedCallbacks{}
	_ pcaptui.ICommandKillError  = urlCopiedCallbacks{}
	_ pcaptui.ICommandTimeout    = urlCopiedCallbacks{}
	_ pcaptui.ICommandWaitTicker = urlCopiedCallbacks{}
)
...
var (
	_ pcaptui.ICommandOutput     = userCopiedCallbacks{}
	_ pcaptui.ICommandError      = userCopiedCallbacks{}
	_ pcaptui.ICommandDone       = userCopiedCallbacks{}
	_ pcaptui.ICommandKillError  = userCopiedCallbacks{}
	_ pcaptui.ICommandTimeout    = userCopiedCallbacks{}
	_ pcaptui.ICommandWaitTicker = userCopiedCallbacks{}
)
...
func openResultsAfterCopy(tmplName string, tocopy string, app gowid.IApp) {
	...
	pcaptui.CopyCommand(strings.NewReader(tocopy), v)
}
...
func processCopyChoices(copyLen int, app gowid.IApp) {
	...
	if len(clips) == 1 {
		...
		pcaptui.CopyCommand(strings.NewReader(clips[0].ClipValue()), userCopiedCallbacks{
			app:     app,
			copyCmd: copyCmd,
			pleaseWaitCallbacks: &pleaseWaitCallbacks{
				w:   pleaseWaitSpinner,
	...
	for _, clip := range clips {
		...
		btn.OnClick(gowid.MakeWidgetCallback("cb", gowid.WidgetChangedFunction(func(app gowid.IApp, w gowid.IWidget) {
			...
			pcaptui.CopyCommand(strings.NewReader(c2.ClipValue()), userCopiedCallbacks{
				app:     app,
				copyCmd: copyCmd,
				pleaseWaitCallbacks: &pleaseWaitCallbacks{
					w:   pleaseWaitSpinner,
...
// getCurrentStructModelWith will return a pcaptui model of a packet section of PDML given a row number,
...
func packetNumberFromCurrentTableRow() (pcaptui.JumpPos, error) {
	tablePos, err := packetListView.FocusXY() // e.g. table position 5
	if err != nil {
		return pcaptui.JumpPos{}, fmt.Errorf("No packet in focus: %v", err)
	}
...
func packetNumberFromTableRow(tableRow int) (pcaptui.JumpPos, error) {
	packetRowId, ok := packetListView.Model().RowIdentifier(tableRow)
	if !ok {
		return pcaptui.JumpPos{}, fmt.Errorf("Error looking up packet at row %v", tableRow)
	}
	...
	if int(packetRowId) >= len(Loader.PsmlData()) {
		return pcaptui.JumpPos{}, fmt.Errorf("Packet %d is not loaded.", packetRowId)
	}

	packetNum, err := strconv.Atoi(Loader.PsmlData()[packetRowId][0])
	if err != nil {
		return pcaptui.JumpPos{}, fmt.Errorf("Unexpected error determining no. of packet %d: %v.", tableRow, err)
	}

	return pcaptui.JumpPos{
		Pos:     packetNum,
		Summary: summary,
	}, nil
}
				...
				if packetListView != nil {
					...
					if err != nil {
						...
						if err != nil {
							...
							globalMarksMap[evk.Rune()] = pcaptui.GlobalJumpPos{
								JumpPos:  jpos,
								Filename: Loader.Pcap(),
							}
							pcaptui.SaveGlobalMarks(globalMarksMap)
...
func MaybeKeepThenRequestLoadPcap(pcapf string, displayFilter string, jump pcaptui.GlobalJumpPos, app gowid.IApp) {
...
func RequestLoadPcap(pcapf string, displayFilter string, jump pcaptui.GlobalJumpPos, app gowid.IApp) {
...
func Build(tty string) (*gowid.App, error) {
	...
	if err != nil {
		return nil, gowid.WithKVs(pcaptui.InternalErr, map[string]interface{}{
			"err": err,
		})
	}
	...
	title := styled.New(text.New(pcaptui.TemplateToString(Templates, "NameVer", TemplateData)), gowid.MakePaletteRef("title"))
		...
		menuutil.SimpleMenuItem{
			...
			CB: func(app gowid.IApp, w gowid.IWidget) {
				...
				if !pcaptui.RunningRemotely() {
					pcaptui.BrowseUrl(pcaptui.UserGuideURL)
				}
				openResultsAfterCopy("UIUserGuide", pcaptui.UserGuideURL, app)
			},
		},
		menuutil.SimpleMenuItem{
			Txt: "FAQ",
			...
			CB: func(app gowid.IApp, w gowid.IWidget) {
				multiMenu1Opener.CloseMenu(generalMenu, app)
				if !pcaptui.RunningRemotely() {
					pcaptui.BrowseUrl(pcaptui.FAQURL)
				}
				openResultsAfterCopy("UIFAQ", pcaptui.FAQURL, app)
			},
		},
		menuutil.MakeMenuDivider(),
		menuutil.SimpleMenuItem{
			...
			CB: func(app gowid.IApp, w gowid.IWidget) {
				multiMenu1Opener.CloseMenu(generalMenu, app)
				if !pcaptui.RunningRemotely() {
					pcaptui.BrowseUrl(pcaptui.BugURL)
				}
				openResultsAfterCopy("UIBug", pcaptui.BugURL, app)
			},
		},
		menuutil.SimpleMenuItem{
			Txt: "Feature Request?",
			...
			CB: func(app gowid.IApp, w gowid.IWidget) {
				multiMenu1Opener.CloseMenu(generalMenu, app)
				if !pcaptui.RunningRemotely() {
					pcaptui.BrowseUrl(pcaptui.FeatureURL)
				}
				openResultsAfterCopy("UIFeature", pcaptui.FeatureURL, app)
			},
		},
		menuutil.MakeMenuDivider(),
		menuutil.SimpleMenuItem{
	...
	// movement to be possible, but in pcaptui it's more often annoying -
	...
	mainviewRows.OnOffsetsSet(gowid.MakeWidgetCallback("cb", func(app gowid.IApp, w gowid.IWidget) {
		pcaptui.SaveOffsetToConfig("mainview", mainviewRows.GetOffsets())
	}))
	...
	altview1Pile.OnOffsetsSet(gowid.MakeWidgetCallback("cb", func(app gowid.IApp, w gowid.IWidget) {
		pcaptui.SaveOffsetToConfig("altviewleft", altview1Pile.GetOffsets())
	}))
	...
	altview1Cols.OnOffsetsSet(gowid.MakeWidgetCallback("cb", func(app gowid.IApp, w gowid.IWidget) {
		pcaptui.SaveOffsetToConfig("altviewright", altview1Cols.GetOffsets())
	}))
	...
	altview2Cols.OnOffsetsSet(gowid.MakeWidgetCallback("cb", func(app gowid.IApp, w gowid.IWidget) {
		pcaptui.SaveOffsetToConfig("altview2vertical", altview2Cols.GetOffsets())
	}))
	...
	altview2Pile.OnOffsetsSet(gowid.MakeWidgetCallback("cb", func(app gowid.IApp, w gowid.IWidget) {
		pcaptui.SaveOffsetToConfig("altview2horizontal", altview2Pile.GetOffsets())
	}))
	...
	if offs, err := pcaptui.LoadOffsetFromConfig("mainview"); err == nil {
		mainviewRows.SetOffsets(offs, app)
	}
	if offs, err := pcaptui.LoadOffsetFromConfig("altviewleft"); err == nil {
		altview1Pile.SetOffsets(offs, app)
	}
	if offs, err := pcaptui.LoadOffsetFromConfig("altviewright"); err == nil {
		altview1Cols.SetOffsets(offs, app)
	}
	if offs, err := pcaptui.LoadOffsetFromConfig("altview2horizontal"); err == nil {
		altview2Pile.SetOffsets(offs, app)
	}
	if offs, err := pcaptui.LoadOffsetFromConfig("altview2vertical"); err == nil {
