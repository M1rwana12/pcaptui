...
import (
	...
	_ "github.com/m1rwana12/pcaptui/assets/statik"
)
			...
			for _, file := range files {
				for _, mode := range s.modes {
					...
					if strings.HasSuffix(file.Name(), suff) {
						...
						if !pcaptui.StringInSlice(m, matches) {
							if strings.Contains(m, s.substr) {
								matches = append(matches, m)
							}
						}
...
func (d setCommand) Run(app gowid.IApp, args ...string) error {
	...
	case 3:
		...
		case "term":
			if err = pcaptui.ValidateTerm(args[2]); err == nil {
...
func newProfileCommand() *profileCommand {
	return &profileCommand{
		pcaptuiProfiles:   profiles.AllNonDefaultNames(),
		wiresharkProfiles: pcaptui.WiresharkProfileNames(),
	}
}
...
func (d profileCommand) Run(app gowid.IApp, args ...string) error {
	...
	case 3:
		switch args[1] {
		case "use":
			if !pcaptui.StringInSlice(args[2], append(d.pcaptuiProfiles, "default")) {
				err = fmt.Errorf("%s is not a valid pcaptui profile", args[2])
				break
			}
		...
		case "link":
			...
			if !pcaptui.StringInSlice(args[2], d.wiresharkProfiles) {
		...
		case "delete":
			if !pcaptui.StringInSlice(args[2], d.pcaptuiProfiles) {
				err = fmt.Errorf("%s is not a valid pcaptui profile", args[2])
				break
			}
...
func (d profileCommand) Arguments(toks []string, app gowid.IApp) []minibuffer.IArg {
	...
	if len(toks) > 0 {
		...
		case "unlink":
		case "use":
			res = append(res, newCachedArg(pref, append(d.pcaptuiProfiles, "default")))
		case "delete":
			res = append(res, newCachedArg(pref, d.pcaptuiProfiles))
		case "link":
...
func (d mapCommand) Run(app gowid.IApp, args ...string) error {
	...
	if len(args) == 3 {
		...
		if len(key1) != 1 {
			...
			pcaptui.AddKeyMapping(pcaptui.KeyMapping{From: key1[0], To: keys2})
			mappings := pcaptui.LoadKeyMappings()
...
func (d unmapCommand) Run(app gowid.IApp, args ...string) error {
	...
	if len(args) != 2 {
		...
		pcaptui.RemoveKeyMapping(key1[0])
		mappings := pcaptui.LoadKeyMappings()
		for _, mapping := range mappings {
			d.w.AddMapping(mapping.From, mapping.To, app)
		}
