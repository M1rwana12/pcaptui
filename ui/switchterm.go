...
func SuggestSwitchingTerm(app gowid.IApp) {
	...
	Yes := dialog.Button{
		...
		Action: gowid.MakeWidgetCallback("exec", gowid.WidgetChangedFunction(func(app gowid.IApp, w gowid.IWidget) {
			pcaptui.ShouldSwitchTerminal = true
	...
	switchTerm = dialog.New(
		framed.NewSpace(paragraph.New(fmt.Sprintf("Pcaptui is running with TERM=%s. The terminal database contains %s. Would you like to switch for a more colorful experience? Pcaptui will need to restart.", term, term256))),
		dialog.Options{
...
// running pcaptui is legible, having upgraded the TERM variable to a 256-color
...
func IsTerminalLegible(app gowid.IApp) {
	...
	saveTerm = dialog.New(
		framed.NewSpace(paragraph.New(fmt.Sprintf("Do you want to save TERM=%s in pcaptui's config to use as the default?", os.Getenv("TERM")))),
		dialog.Options{
	...
	tw := func(count int) *paragraph.Widget {
		return paragraph.New(fmt.Sprintf("Is the terminal legible? If no selection is made, pcaptui will revert to its original TERM setting in %d seconds.", secs))
	}
