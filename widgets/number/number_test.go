...
func TestNumber1(t *testing.T) {
	...
	clickat := func(x, y int) {
		...
		gwtest.D.SetLastMouseState(gowid.MouseState{MouseLeftClicked: true, MouseLastClickedTime: time.Now()})
		w.UserInput(evunclick(x, y), sz, gowid.Focused, gwtest.D)
		gwtest.D.SetLastMouseState(gowid.MouseState{MouseLastClickedTime: time.Now()})
	}
