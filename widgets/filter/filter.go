...
// Package filter provides a pcaptui-specific edit widget which changes
...
func New(name string, opt Options) *Widget {
	...
	for _, k := range vim.AllDownKeys {
		if !pcaptui.KeyPressIsPrintable(gowid.Key(k)) {
...
func (w *Widget) UserInput(ev interface{}, size gowid.IRenderSize, focus gowid.Selector, app gowid.IApp) bool {
	if evk, ok := ev.(*tcell.EventKey); ok {
		if evk.Key() == tcell.KeyTAB || (vim.KeyIn(evk, vim.AllDownKeys) && !pcaptui.KeyPressIsPrintable(evk)) {
			return false
		}
	}
...
func (w *activatorWidget) UserInput(ev interface{}, size gowid.IRenderSize, focus gowid.Selector, app gowid.IApp) bool {
	...
	if ev, ok := ev.(*tcell.EventKey); ok && !w.active {
		if vim.KeyIn(ev, vim.AllDownKeys) && !pcaptui.KeyPressIsPrintable(ev) {
			w.active = true
			return true
		} else {
			return false
	...
	if !res {
		if ev, ok := ev.(*tcell.EventKey); ok && w.active {
			if vim.KeyIn(ev, vim.AllUpKeys) && !pcaptui.KeyPressIsPrintable(ev) {
				w.active = false
				return true
			} else {
				return false
...
func (f *DisplayFilterValidator) Validate(filter string) {
	...
	f.Cmd = exec.Command(pcaptui.TSharkBin(), []string{"-Y", filter, "-r", pcaptui.CacheFile("empty.pcap")}...)
	err = f.Cmd.Run()
