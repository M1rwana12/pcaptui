...
// output. Because it's built for pcaptui, it also allows styling to be
// applied to ranges of data intended to correspond to packet structure selected
// in another pcaptui view.
package hexdumper
...
import (
	...
	"github.com/m1rwana12/pcaptui/widgets/renderfocused"
...
func (w *Widget) InHex() bool {
	...
	if len(fp) < 3 {
		panic(errors.WithStack(gowid.WithKVs(pcaptui.BadState, map[string]interface{}{"focus path": fp})))
	}
	return fp[0] == 3
}
...
func (w *Widget) Position() int {
	fp := gowid.FocusPath(w.w)
	if len(fp) < 3 {
		panic(gowid.WithKVs(pcaptui.BadState, map[string]interface{}{"focus path": fp}))
	}
	if fp[0] == 3 {
		// in hex
		x := fp[2].(int)
