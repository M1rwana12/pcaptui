...
func New() (*ConfigWatcher, error) {
	...
	if err := watcher.Add(pcaptui.ConfFile("pcaptui.toml")); err != nil && !os.IsNotExist(err) {
		return nil, err
	}
...
func (c *ConfigWatcher) Close() {
	...
	// is safe because I know, at this point, there are no other readers because pcaptui
