...
func MakeUsefulError(cmd IBasicCommand, err error) gowid.KeyValueError {
	return gowid.WithKVs(pcaptui.BadCommand, map[string]interface{}{
		"command": cmd.String(),
		"error":   err,
...
type IPcapCommand interface {
	IBasicCommand
	StdoutReader() (io.ReadCloser, error) // pcaptui will .Read() from result
}
...
// Renew is called when a new pcap is loaded from an open pcaptui session i.e. pcaptui
...
// IMainRunner is implemented by a type that runs a closure on pcaptui's main loop
// (via gowid's App.Run)
...
func (c *PdmlLoader) loadPcapSync(row int, visible bool, ps iPdmlLoaderEnv, cb interface{}, app gowid.IApp) {
	...
	pcaptui.TrackedGo(func() {
		...
		pcaptui.TrackedGo(func() {
			...
			killPcap := func() {
				err := pcaptui.KillIfPossible(pcapCmd)
				if err != nil {
			...
			killPdml := func() {
				err = pcaptui.KillIfPossible(pdmlCmd)
				if err != nil {
		...
		Loop:
			...
			ps.MainRun(gowid.RunFunction(func(gowid.IApp) {
				...
				// that pcaptui doesn't think that there are only 900 packets, because that's what's
		...
		pcaptui.TrackedGo(func() {
			...
			for {
				...
				if packet, done := hex.Line(line); done {
...
// specific loading bug in pcaptui due to an optimized packet capture
// process. To capture packets, pcaptui runs itself with a special env var
...
// extcap sources. The problem is that pcaptui needs a heuristic for when
...
// that with the UI). So pcaptui waits for a WRITE to the pcap generated by
...
func (p *PsmlLoader) loadPsmlSync(iloader *InterfaceLoader, e iPsmlLoaderEnv, cb interface{}, app gowid.IApp) {
	...
	pcaptui.TrackedGo(func() {
		...
		pcaptui.TrackedGo(func() {
			...
			kill := func() {
				err := pcaptui.KillIfPossible(psmlCmd)
				if err != nil {
		...
		if p.ReadingFromFifo() {
			...
			pcaptui.TrackedGo(func() {
				...
				kill := func() {
					err := pcaptui.KillIfPossible(tailCmd)
					if err != nil {
...
func (i *InterfaceLoader) loadIfacesSync(e iIfaceLoaderEnv, cb interface{}, app gowid.IApp) {
	...
	pcaptui.TrackedGo(func() {
		defer func() {
			// if psrc is a PipeSource, then we open /dev/fd/3 in pcaptui, and reroute descriptor
			// stdin to number 3 when pcaptui starts. So to kill the process writing in, we need
		...
		killIface := func() {
			err = pcaptui.KillIfPossible(i.ifaceCmd)
			if err != nil {
		...
		for {
			...
			case err = <-ifaceTermChan:
				...
				if !e.PsmlStoppedDeliberately() && err != nil {
					if _, ok := err.(*exec.ExitError); ok {
						// This could be if pcaptui is started like this: cat nosuchfile.pcap | pcaptui -i -
...
// which will trigger the psml reading process to shut down, and pcaptui
...
func TempPcapFile(tokens ...string) string {
	...
	return filepath.Join(pcaptui.PcapDir(), fmt.Sprintf("%s--%s.pcap",
		tokenClean,
		pcaptui.DateStringForFilename(),
	))
}
