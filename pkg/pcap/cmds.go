...
func (c *Command) Start() error {
	...
	c.Cmd.Stderr = io.MultiWriter(pw, pcaptui.ErrLogger("cmd", c.Path))
...
func (c Commands) Iface(ifaces []string, captureFilter string, tmpfile string) IBasicCommand {
	...
	res := &Command{
		Cmd: exec.Command(pcaptui.CaptureBin(), args...),
	}
	// This tells pcaptui to start in a special capture mode. It allows pcaptui
	// to run itself like this:
	//
	// pcaptui -i eth0 -w foo.pcap
	//
	...
	// (extcap interfaces).
	res.Cmd.Env = append(os.Environ(), "PCAPTUI_CAPTURE_MODE=1")
...
func (c Commands) Pcap(pcap string, displayFilter string) IPcapCommand {
	...
	args = append(args, c.Args...)
	return &Command{Cmd: exec.Command(pcaptui.TSharkBin(), args...)}
}
