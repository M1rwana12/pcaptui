...
// Test using same commands that pcaptui uses - load 1.pcap. Also tests re-use of a loader.
func TestRealProcs(t *testing.T) {
	loader := NewPcapLoader(Commands{})
	// Make sure we can re-use the same loader, because that's what pcaptui does
	for _, _ = range []int{1, 2, 3} {
		...
		<-pdmlFinChan
		assert.Equal(t, LoaderState(LoadingPdml), loader.State())
		loader.SetState(loader.State() & ^LoadingPdml) // manually reset state, pcaptui handles this
