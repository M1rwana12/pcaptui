...
func DeleteCachedFields() error {
	return os.Remove(pcaptui.CacheFile("tsharkfieldsv3.gob.gz"))
}

// Can be run asynchronously.
// This ought to use interfaces to make it testable.
func (w *TSharkFields) Init() error {
	newer, err := pcaptui.FileNewerThan(pcaptui.CacheFile("tsharkfieldsv3.gob.gz"), pcaptui.DirOfPathCommandUnsafe(pcaptui.TSharkBin()))
	if err == nil && newer {
		f := &FieldsAndProtos{
		...
		err = pcaptui.ReadGob(pcaptui.CacheFile("tsharkfieldsv3.gob.gz"), f)
		if err == nil {
			w.ser = f
	...
	err = pcaptui.WriteGob(pcaptui.CacheFile("tsharkfieldsv3.gob.gz"), w.ser)
	if err != nil {
		return err
	}
...
func (w *TSharkFields) InitNoCache() error {
	cmd := exec.Command(pcaptui.TSharkBin(), []string{"-G", "fields"}...)
