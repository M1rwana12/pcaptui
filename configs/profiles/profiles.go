...
func ReadDefaultConfig(dir string) error {
	return readConfig(vDefault, dir, "pcaptui", true)
}
...
func readConfig(v *viper.Viper, dir string, base string, createIfNecessary bool) error {
	...
	v.SetConfigName(base) // no need to include file extension - looks for file called pcaptui.ini for example
	v.AddConfigPath(dir)
	...
	if err == nil {
		for _, file := range files {
			if file.Name() != "default" {
				if _, err := os.Stat(filepath.Join(profPath, file.Name(), "pcaptui.toml")); err == nil {
...
func Use(name string) error {
	...
	if err := readConfig(vNew, dir, "pcaptui", false); err != nil {
		return err
	}
