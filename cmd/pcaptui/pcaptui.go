...
import (
	...
	"github.com/m1rwana12/pcaptui/pkg/confwatcher"
	"github.com/m1rwana12/pcaptui/pkg/convs"
	"github.com/m1rwana12/pcaptui/pkg/fields"
	...
	"github.com/m1rwana12/pcaptui/pkg/tailfile"
	"github.com/m1rwana12/pcaptui/pkg/tty"
	"github.com/m1rwana12/pcaptui/ui"
	"github.com/m1rwana12/pcaptui/widgets/filter"
	"github.com/m1rwana12/pcaptui/widgets/wormhole"
...
func main() {
	...
	stats.Goroutinewg = &ensureGoroutinesStopWG
	...
	if !pcaptui.ShouldSwitchTerminal && !pcaptui.ShouldSwitchBack {
		os.Exit(res)
	}

	os.Clearenv()
	for _, e := range pcaptui.OriginalEnv {
	...
	if err != nil {
		log.Warnf("Unexpected error determining pcaptui executable: %v", err)
		os.Exit(1)
	}

	switch {
	case pcaptui.ShouldSwitchTerminal:
		os.Setenv("PCAPTUI_ORIGINAL_TERM", os.Getenv("TERM"))
		os.Setenv("TERM", fmt.Sprintf("%s-256color", os.Getenv("TERM")))
	case pcaptui.ShouldSwitchBack:
		os.Setenv("TERM", os.Getenv("PCAPTUI_ORIGINAL_TERM"))
		os.Setenv("PCAPTUI_ORIGINAL_TERM", "")
	}
	...
	if err != nil {
		log.Warnf("Unexpected error exec-ing pcaptui %s: %v", exe, err)
		res = 1
	}
...
func cmain() int {
	...
	pcaptui.OriginalEnv = os.Environ()
	...
	// ctrl-c and ctrl-\ to pcaptui as keypress events. But we slightly
	...
	// recognize ctrl-z in pcaptui and issue a SIGSTOP to getpid() from
	// pcaptui but this wouldn't stop other processes in a pcaptui
	// pipeline e.g.
	//
	// tcpdump -i eth0 -w - | pcaptui -i -
	//
	...
	// as root and pcaptui is not, tcpdump will not be suspended. If
	...
	// if help is on, then we'll detect it, parse the flags as pcaptui, then
	...
	if err != nil {
		// If it's because of --help, then skip the tty check, and display pcaptui's help. This
		// ensures we don't display a useless help, and further that you can pipe pcaptui's help
	...
	// On Windows, pcaptui itself is used to tail the pcap generated by dumpcap, and the output
	...
	// If this variable is set, it's set by pcaptui internally, and pcaptui is guaranteed to
	...
	if os.Getenv("PCAPTUI_CAPTURE_MODE") == "1" {
		err = system.DumpcapExt(pcaptui.DumpcapBin(), pcaptui.TSharkBin(), os.Args[1:]...)
		if err != nil {
			return 1
		} else {
			return 0
	...
	if passthru &&
		...
		tsharkBin, kverr := pcaptui.TSharkPath()
		if kverr != nil {
			fmt.Fprint(os.Stderr, kverr.KeyVals["msg"].(string))
			return 1
		}

		// The same check the UI path makes. Without it, piping pcaptui's
		// output would quietly restore the behaviour this option exists to
		// prevent: a mistyped keylog path that decrypts nothing and says
		// nothing.
		if tsopts.TLSKeylog != "" {
			if err := cli.CheckKeylog(tsopts.TLSKeylog); err != nil {
				fmt.Fprintf(os.Stderr, "%v\n", err)
				return 1
			}
		}

		// Drops pcaptui's own flags and turns --tls-keylog into the tshark
		// preference, so decryption survives being piped.
		args := append([]string{tsharkBin}, cli.TsharkArgs(os.Args[1:])...)
	...
	var opts cli.Pcaptui

	// Parse the args now as intended for pcaptui
	...
	if len(opts.Version) > 0 {
		...
		if len(opts.Version) > 1 {
			if tsharkBin, kverr := pcaptui.TSharkPath(); kverr != nil {
				fmt.Fprint(os.Stderr, kverr.KeyVals["msg"].(string))
				res = 1
			} else {
				if ver, err := pcaptui.TSharkVersion(tsharkBin); err != nil {
	...
	// argument is a pcap file e.g. pcaptui foo.pcap
	if pcapf == "" && len(opts.Ifaces) == 0 {
		pcapf = string(opts.Args.FilterOrPcap)
		// `pcaptui` => `pcaptui -i 1` (livecapture on default interface if no args)
		if pcapf == "" {
			if pcaptui.IsTerminal(os.Stdin.Fd()) {
				pfile, err := system.PickFile()
				switch err {
				case nil:
					// We're on termux/android, and we were given a file. Note that termux
					// makes a copy, so we ought to clean that up when pcaptui terminates.
				...
				// $ cat foo.pcap | pcaptui
				// # use stdin - maps to
				// $ cat foo.pcap | pcaptui -r -
	...
	for pi, psrc := range psrcs {
		...
		case psrc.Name() == "-":
			...
			if pcaptui.IsTerminal(os.Stdin.Fd()) {
				...
				fmt.Fprintf(os.Stderr, "Perhaps you intended to pipe packet input to pcaptui?\n")
				return 1
			}
			if runtime.GOOS != "windows" {
				...
				fmt.Fprintf(os.Stderr, "Sorry, pcaptui does not yet support piped input on Windows.\n")
				return 1
			}
		default:
			...
			if stat.Mode()&os.ModeNamedPipe != 0 {
				// If pcaptui was invoked with -r myfifo, switch to -i myfifo, which tshark uses. This
				// also puts pcaptui in "interface" mode where it assumes the source is unbounded
				...
				if pcapffile, err := os.Open(psrc.Name()); err != nil {
					...
					// it and the pcaptui user not, but unlikely.
					fmt.Fprintf(os.Stderr, "Error reading file %s: %v.\n", psrc.Name(), err)
					return 1
				} else {
					pcapffile.Close()
	...
	pcaptui.ReverseStringSlice(filterArgs)
	...
	if !opts.LogTty {
		logfile := pcaptui.CacheFile("pcaptui.log")
	...
	if debug {
		for _, addr := range pcaptui.LocalIPs() {
	...
	for _, dir := range []string{pcaptui.CacheDir(), pcaptui.DefaultPcapDir(), pcaptui.PcapDir()} {
	...
	emptyPcap := pcaptui.CacheFile("empty.pcap")
	if _, err := os.Stat(emptyPcap); os.IsNotExist(err) {
		err = pcaptui.WriteEmptyPcap(emptyPcap)
		if err != nil {
	...
	if !pcaptui.StringInSlice(tsharkBin, valids) {
		tver, err := pcaptui.TSharkVersion(tsharkBin)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Could not determine tshark version: %v\n", err)
			return 1
		}
		// This is the earliest version I could determine gives reliable results in pcaptui.
		// tshark compiled against tag v1.10.1 doesn't populate the hex view.
		mver, _ := semver.Make("1.10.2")
		if tver.LT(mver) {
			fmt.Fprintf(os.Stderr, "pcaptui will not operate correctly with a tshark older than %v (found %v)\n", mver, tver)
			return 1
		}
	...
	if !pcaptui.StringInSlice(tsharkBin, colorTsharks) {
		ui.PacketColorsSupported, err = pcaptui.TSharkSupportsColor(tsharkBin)
		if err != nil {
	...
	for pi, psrc := range psrcs {
		...
		if checkInterfaceName {
			if systemInterfaces == nil {
				systemInterfaces, err = pcaptui.Interfaces()
				if err != nil {
	...
	defer func() {
		...
		// capture at the end of running pcaptui.
	...
	defer func() {
		if ifaceExitCode != 0 {
			...
			if stderr.Len() != 0 {
				// The default capture bin is pcaptui itself, with a special environment
				...
				// But telling the user the capture process is "pcaptui" is misleading.
				cbin, err1 := filepath.Abs(filepath.FromSlash(pcaptui.CaptureBin()))
				def, err2 := filepath.Abs("pcaptui")
	...
	// TLS decryption. tshark accepts a keylog path that does not exist,
	// exits zero and decrypts nothing, so check it here - otherwise a
	// mistyped path looks exactly like traffic whose keys are missing.
	keylog := opts.TLSKeylog
	if keylog == "" {
		keylog = profiles.ConfString("main.tls-keylog", "")
	}
	if keylog != "" {
		if err := cli.CheckKeylog(keylog); err != nil {
			fmt.Fprintf(os.Stderr, "%v\n", err)
			return 1
		}
		tsharkArgs = append(tsharkArgs, cli.KeylogArg(keylog)...)
		log.Infof("Decrypting TLS with keys from %s", keylog)
	}
	...
	// are using a pcaptui theme that expresses colors in RGB style (#7799AA), and
	// pcaptui is running in a 256-color terminal, then pcaptui will find the closest
	// match for the RGB color in the 256 color-space. But pcaptui assumes that colors
	...
	if gowid.IgnoreBase16 {
		...
		// If main.respect-colorterm=true then pcaptui will leave COLORTERM set and use
		...
		// pcaptui will fall back to 256-colors if base16 is detected because I can
		// programmatically avoid choosing colors 0-21 for anything pcaptui needs.
	...
	if app, err = ui.Build(usetty); err != nil {
		...
		if _, ok := pcaptui.RootCause(err).(*exec.ExitError); ok {
			fmt.Fprintf(os.Stderr, "Pcaptui could not recognize your terminal. Try changing $TERM.\n")
		}
		return 1
	}
	...
	if waitingForPackets {
		fmt.Fprintf(os.Stderr, "(The pcaptui UI will start when packets are detected on %s...)\n",
	...
	if len(fileSrcs) > 0 {
		...
		for _, psrc := range psrcs {
			if psrc.IsInterface() {
				if ifaceExitCode, ifaceErr = pcaptui.RunForStderr(
					pcaptui.CaptureBin(),
					[]string{"-i", psrc.Name(), "-a", "duration:1"},
					append(os.Environ(), "PCAPTUI_CAPTURE_MODE=1"),
					stderr,
				); ifaceExitCode != 0 {
					return 1
				}
				...
				// this lets pcaptui start up more quickly.
				break
			}
		}
	...
	currentlyInactive := false // True if the timer has fired and pcaptui is in "inactive" state
	...
	if shotScreen != nil {
		if opts.Screencast {
			go captureCast(app, shotScreen, opts.Screenshot, opts.ScreenshotKeys)
		} else {
	...
	for {
		...
		case <-checkPcapCacheChan:
			...
			if profiles.ConfBool("main.use-tshark-temp-for-pcap-cache", false) {
				log.Infof("Pcaptui does not own the pcap temp dir %s; skipping size check", pcaptui.PcapDir())
			} else {
				pcaptui.PrunePcapCache()
			}
		...
		case <-ui.StartUIChan:
			...
			if runtime.GOOS != "windows" {
				if app.GetColorMode() == gowid.Mode8Colors {
					...
					// just load up pcaptui normally with no further interruption.
					if _, exists := os.LookupEnv("PCAPTUI_ORIGINAL_TERM"); !exists {
						if !profiles.ConfBool("main.disable-term-helper", false) {
							err = pcaptui.Does256ColorTermExist()
							if err != nil {
				...
				} else if os.Getenv("PCAPTUI_ORIGINAL_TERM") != "" {
					time.AfterFunc(time.Duration(3)*time.Second, func() {
						app.Run(gowid.RunFunction(func(app gowid.IApp) {
		...
		case sig := <-sigChan:
			if system.IsSigTSTP(sig) {
				...
				if debug {
					pcaptui.ProfileCPUFor(20)
				} else {
					log.Infof("SIGUSR1 ignored by pcaptui - see the --debug flag")
				}
				...
				if debug {
					...
					log.Infof("SIGUSR2 ignored by pcaptui - see the --debug flag")
				}
