...
wait_for_load() {
    rm ~/.cache/termshark/pcaptui.log > /dev/null 2>&1
    tail -F ~/.cache/termshark/pcaptui.log 2> /dev/null | while [ 1 ] ; do read ; echo Log: $REPLY 1>&2 ; grep "Load operation complete" <<<$REPLY && break ; done
}
